...
2. 校验哈希（以 Release 页面标注为准）：
   ...
   # v0.0.10-pre.4 应输出 c430b164ebf639f6fcc004a93095ff0d80ee1bb7b63644f525eab234792525de
   ```
3. 放入服务器 `mods/` 目录。
...
| 现象 | 原因与处理 |
|---|---|
| 客户端提示“缺少模组/需安装模组”无法加入 | ① pre.1 之前：`mods.toml` 缺 `IGNORE_SERVER_VERSION`；② pre.2/pre.3：mod 列表检查已通过，但服务器注册的网络通道 `numen_api:main` 会被未安装模组的客户端硬性拒绝（`mismatched mod channel list`，无豁免）。**v0.0.10-pre.4 起服务端不再注册任何通道，客户端无需安装即可加入** |
