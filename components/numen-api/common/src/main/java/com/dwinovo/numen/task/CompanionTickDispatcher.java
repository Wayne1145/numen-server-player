...
public final class CompanionTickDispatcher {
    ...
    public static void tick(MinecraftServer server) {
        ...
        for (ServerPlayer p : server.getPlayerList().getPlayers()) {
            // A degraded body is paused: skip its task brain entirely (its movement pass is
            // skipped in NumenPlayer.tick) so repeated failures can't keep driving broken work.
            if (p instanceof NumenPlayer ap && !ap.isDegraded()) {
    ...
    /**
     * Stop a companion's tasks because it just entered the DEGRADED state (repeated tick failures).
     * Same cancel path as an owner Stop — cancel the pending queue and the running task, release the
     * MAINHAND intent pin — but never creates a brain (a body with no tasks has nothing to stop).
     */
    public static void stopTasksForDegraded(NumenPlayer player) {
        CompanionBrain brain = BRAINS.get(player.getUUID());
        if (brain == null) return;
        brain.queue.cancelAll("companion degraded: repeated tick errors");
