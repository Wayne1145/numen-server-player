 ...
 * <ul>
 *   <li><b>Forge(1.20.1)</b> — Forge 补丁把取数改成调用
 *       {@code SoundInstance.getStream(SoundBufferLibrary, Sound, boolean)}
 *       补丁钩子,工厂返回覆写了该钩子的 {@code ForgeEntityVoiceSound},零 mixin;</li>
 *   <li><b>Fabric</b> — Fabric API 的 sound-api-v1 已把 vanilla 形状的
 *       {@code SoundBufferLibrary.getStream(ResourceLocation, boolean)} 调用
 *       重定向到接口注入的 {@code FabricSoundInstance.getAudioStream},
 *       工厂返回覆写了该钩子的 {@code FabricEntityVoiceSound},零自有 mixin。</li>
 * </ul>
...
public class EntityVoiceSound extends AbstractTickableSoundInstance implements VoicePcmSource {
     ...
     * Forge 由 {@code ForgeEntityVoiceSound} 覆写的 {@code getStream} 补丁钩子
     * 调用;Fabric 由 {@code FabricEntityVoiceSound} 覆写的 Fabric API
     * {@code getAudioStream} 钩子调用。
     */
    @Override
