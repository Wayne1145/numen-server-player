...
public final class ExecHarness implements Movement.ExecutionDelegate {
    ...
    public void commit() {
        ...
        if (sneak) {
            // 潜行冲量乘数(1.20.1 无 SNEAKING_SPEED 属性):基值 0.3,
            // 迅捷潜行附魔每级 +0.15,封顶 1.0——vanilla LocalPlayer 同款公式。
            float sneakSpeed = net.minecraft.util.Mth.clamp(
                    0.3f + net.minecraft.world.item.enchantment.EnchantmentHelper.getEnchantmentLevel(
                            net.minecraft.world.item.enchantment.Enchantments.SWIFT_SNEAK, player) * 0.15f,
                    0.0f, 1.0f);
            forward *= sneakSpeed;
            strafe *= sneakSpeed;
        }
